...
4. **Dispatch Per-Page Checks**: For each successfully crawled page, invoke `crawl-render-audit`, `freshness-corroboration`, `entity-trust`, and `engagement-audit` sub-skills with `{url, page_data}`. Collect returned finding records.
...
5. **Site-Wide Aggregation** (run once, not per page):
   ...
   - **Entity corroboration gap**: Delegate to `entity-trust` for site-wide `sameAs` and `disambiguatingDescription` presence across all Organization/Brand JSON-LD.
...
6. **Correlate & Emit**: Deduplicate findings by `(url, check_id)`. Sort by severity (`critical → high → medium`). Apply severity rubric from `references/severity_rubric.md`. Write JSON to `output_path`.

## References
- [`references/severity_rubric.md`](references/severity_rubric.md) — Confidence-to-severity mapping and dedup logic applied at compile time.
- [`references/remediation_catalog.md`](references/remediation_catalog.md) — `suggested_action` text catalog per finding signal type.
- [`references/checklist.md`](references/checklist.md) — Pre-release structural and functional validation checklist.
